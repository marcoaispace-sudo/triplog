...
export default function Home(){
  ...
  const hkdSpent=useMemo(()=>expenses.filter(item=>item.currency==="HKD").reduce((sum,item)=>sum+Number(item.amount||0),0),[expenses]);
  const hkdRemaining=Math.max(0,activeTrip.budget-hkdSpent);
  const budgetPercent=activeTrip.budget?Math.min(100,Math.round(hkdSpent/activeTrip.budget*100)):0;
