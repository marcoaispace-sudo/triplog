...
import { searchHotels, type HotelPlaceResult } from "./google-places";
...
export default function Home(){
  ...
  const dayItems=useMemo(()=>(activeTrip.itinerary??[]).filter(item=>item.plan!=="backup"&&item.date===shownDate).sort((a,b)=>a.time.localeCompare(b.time)),[activeTrip.itinerary,shownDate]);
  const backupItems=useMemo(()=>(activeTrip.itinerary??[]).filter(item=>item.plan==="backup").sort((a,b)=>a.date.localeCompare(b.date)||a.time.localeCompare(b.time)),[activeTrip.itinerary]);
  ...
  const openNewItinerary=()=>{setEditingItineraryId(null);setModal("itinerary")};
  const openEditItinerary=(item:ItineraryItem)=>{setEditingItineraryId(item.id);setModal("itinerary")};
  const saveItineraryItem=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const data=new FormData(event.currentTarget);const date=String(data.get("date")||shownDate);const time=String(data.get("time")||"");const title=String(data.get("title")||"").trim();const type=String(data.get("type")||"行程");const note=String(data.get("note")||"").trim();const location=String(data.get("location")||"").trim();const plan=String(data.get("plan")||"main") as "main"|"backup";if(!date||!time||!title){toast("請填寫日期、時間及名稱");return}const color:ItineraryItem["color"]=["餐廳","餐飲"].includes(type)?"coral":type==="交通"?"gold":"blue";const item:ItineraryItem={id:editingItineraryId??crypto.randomUUID(),date,time,title,note,type,color,location,plan};setTrips(current=>current.map(trip=>{if(trip.id!==activeTrip.id)return trip;const itinerary=trip.itinerary??[];return {...trip,itinerary:editingItineraryId?itinerary.map(existing=>existing.id===editingItineraryId?item:existing):[...itinerary,item]}}));setSelectedDate(date);setEditingItineraryId(null);setModal(null);setActive("行程");toast(editingItineraryId?"行程內容已更新並準備同步":plan==="backup"?"已加入備案行程":"行程已加入並準備同步")};
  ...
  const moveBackupToMain=(id:string,date=shownDate)=>{setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,itinerary:(trip.itinerary??[]).map(item=>item.id===id?{...item,plan:"main",date}:item)}:trip));setDraggingBackupId(null);setBackupOverMain(false);toast(`已加入第${Math.max(1,dates.indexOf(date)+1)}日主線行程`)};
  ...
  const lookupHotel=async(name:string)=>{
    ...
    catch(error){setHotelLookupMessage(error instanceof Error?error.message:"酒店搜尋失敗，請手動輸入")}
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="首頁"&&<>
      ...
      <section className="day-section"><div className="section-heading day-heading"><div><p>第{selectedDayNumber}日安排</p><h3>{activeTrip.destination}行程</h3></div><button className="recommend-button" onClick={()=>toast("附近推介將會在下一階段接入酒店附近搜尋")}><Icon name="pin" size={16}/>附近推介</button></div><Timeline items={dayItems} onMap={openMap}/></section>
    </>}
