...
type ModalType = "ticket"|"hotel"|"receipt"|"expense"|"trip"|"itinerary"|"ocr"|"recommend"|"companion"|null;
...
type Expense = {id:string;date:string;merchant:string;category:string;amount:number;currency:string;paidBy:string;splitMode:"none"|"equal";splitWith:string[];note:string;createdAt:string};
...
function ExpensePieChart({items,currency}:{items:Expense[];currency:string}){
  const categories=Object.entries(items.filter(item=>item.currency===currency).reduce<Record<string,number>>((totals,item)=>{totals[item.category]=(totals[item.category]??0)+Number(item.amount||0);return totals},{})).filter(([,amount])=>amount>0).sort((a,b)=>b[1]-a[1]);
  const total=categories.reduce((sum,[,amount])=>sum+amount,0);
  return <section className="expense-chart-card" aria-label={`${currency} 開支分類圓形圖`}><div className="pie-wrap"><svg className="pie-chart" viewBox="0 0 100 100" role="img" aria-label={`${currency} 開支分類`}><circle className="pie-track" cx="50" cy="50" r="38" pathLength="100"/>{categories.map(([category,amount],index)=>{const percent=total?amount/total*100:0;const offset=total?categories.slice(0,index).reduce((sum,[,previousAmount])=>sum+previousAmount,0)/total*100:0;return <circle key={category} className="pie-segment" cx="50" cy="50" r="38" pathLength="100" stroke={EXPENSE_CATEGORY_COLORS[category]??EXPENSE_CATEGORY_COLORS["其他"]} strokeDasharray={`${percent} ${100-percent}`} strokeDashoffset={-offset}/>})}</svg><div className="pie-center"><small>{currency}</small><strong>{total.toLocaleString(undefined,{maximumFractionDigits:2})}</strong></div></div><div className="pie-legend">{categories.map(([category,amount])=><div key={category}><span style={{background:EXPENSE_CATEGORY_COLORS[category]??EXPENSE_CATEGORY_COLORS["其他"]}}/><strong>{category}</strong><small>{amount.toLocaleString(undefined,{maximumFractionDigits:2})}・{total?Math.round(amount/total*100):0}%</small></div>)}</div></section>
}
...
export default function Home(){
  ...
  const [expenseDraft,setExpenseDraft]=useState<ExpenseDraft>({date:localTodayDateOnly(),merchant:"",category:"餐飲",amount:0,currency:"HKD",paidBy:"",splitMode:"none",splitWith:[],note:""});
  const [receiptBusy,setReceiptBusy]=useState(false);
  const [receiptProgress,setReceiptProgress]=useState(0);
  const [receiptRawText,setReceiptRawText]=useState("");
  const [receiptPreviewUrl,setReceiptPreviewUrl]=useState("");
  const [chartCurrency,setChartCurrency]=useState("HKD");
  ...
  const expenseTotals=useMemo(()=>Object.entries(expenses.reduce<Record<string,number>>((totals,item)=>{totals[item.currency]=(totals[item.currency]??0)+Number(item.amount||0);return totals},{})).sort(([a],[b])=>a.localeCompare(b)),[expenses]);
  const expenseCurrencies=expenseTotals.map(([currency])=>currency);
  const activeChartCurrency=expenseCurrencies.includes(chartCurrency)?chartCurrency:(expenseCurrencies[0]??"HKD");
  ...
  const pick=(type:ModalType)=>{
    ...
    if(type==="receipt"){
      ...
      setEditingExpenseId(null);setExpenseDraft({date:dates.includes(today)?today:activeTrip.startDate,merchant:"",category:"餐飲",amount:0,currency:"HKD",paidBy:payer,splitMode:"none",splitWith:[],note:""});setReceiptRawText("");setReceiptProgress(0);setModal("expense");
  ...
  const openEditExpense=(expense:Expense)=>{setEditingExpenseId(expense.id);setExpenseDraft({...expense});setPicked("");setPickedFile(null);setReceiptRawText("");setReceiptProgress(0);if(receiptPreviewUrl){URL.revokeObjectURL(receiptPreviewUrl);setReceiptPreviewUrl("")}setModal("expense")};
  ...
  const recognizeReceipt=async()=>{
    ...
    try{
      ...
      setExpenseDraft(current=>({...current,merchant:fields.merchant||current.merchant,date:fields.date||current.date,amount:Number(fields.amount)||current.amount,currency:fields.currency||current.currency}));
  ...
  const saveExpense=(event:FormEvent<HTMLFormElement>)=>{
    ...
    const expense:Expense={...expenseDraft,merchant:expenseDraft.merchant.trim(),amount,id:editingExpenseId??crypto.randomUUID(),createdAt:editingExpenseId?(expenses.find(item=>item.id===editingExpenseId)?.createdAt??new Date().toISOString()):new Date().toISOString()};
    ...
    setModal(null);setEditingExpenseId(null);setPickedFile(null);setPicked("");setReceiptRawText("");if(receiptPreviewUrl){URL.revokeObjectURL(receiptPreviewUrl);setReceiptPreviewUrl("")}toast(editingExpenseId?"帳目已更新並準備同步":"帳目已加入並準備同步");
  };
  const deleteExpense=(expense:Expense)=>{if(!window.confirm(`確定刪除「${expense.merchant}」${expense.currency} ${expense.amount.toLocaleString()}？`))return;setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,expenses:(trip.expenses??[]).filter(item=>item.id!==expense.id)}:trip));toast("帳目已刪除")};
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="記帳"&&<section className="screen-content ledger-screen"><div className="page-title"><div><p>{activeTrip.name}</p><h2>旅行記帳</h2></div><div className="ledger-add-actions"><button className="round-add" onClick={()=>pick("receipt")} aria-label="手動新增帳目"><Icon name="plus"/></button><button className="round-add coral" onClick={()=>pick("receipt")} aria-label="拍攝收據入帳"><Icon name="camera"/></button></div></div>{expenses.length===0?<div className="empty-ledger"><Icon name="receipt"/><strong>尚未有帳目</strong><p>可手動新增、即時拍照，或從相簿選擇收據辨認。</p></div>:<><section className="expense-total-panel"><div><p>開支總數</p><h3>{expenses.length} 筆已記錄開支</h3><small>不同貨幣分開計算，不會混合成錯誤總額。</small></div><div className="currency-total-grid">{expenseTotals.map(([currency,total])=><article key={currency}><span>{currency}</span><strong>{total.toLocaleString(undefined,{maximumFractionDigits:2})}</strong></article>)}</div></section><div className="expense-chart-heading"><div><p>開支分類</p><h3>分類圓形圖</h3></div>{expenseCurrencies.length>1&&<div className="chart-currency-tabs" aria-label="選擇圓形圖貨幣">{expenseCurrencies.map(currency=><button key={currency} className={activeChartCurrency===currency?"selected":""} onClick={()=>setChartCurrency(currency)}>{currency}</button>)}</div>}</div><ExpensePieChart items={expenses} currency={activeChartCurrency}/><div className="ledger-summary"><span>{expenses.length} 筆帳目</span><strong>{expenseCurrencies.length} 種貨幣</strong></div><div className="expense-list">{expenses.map(expense=><article className="expense-card" key={expense.id}><span className={`expense-category ${expense.category==="餐飲"?"food":""}`}><Icon name={expense.category==="交通"?"plane":expense.category==="住宿"?"hotel":expense.category==="購物"?"wallet":"receipt"} size={17}/></span><div className="expense-copy"><small>{dateLabel(expense.date)}・{expense.category}</small><strong>{expense.merchant}</strong><span>{expense.paidBy} 付款{expense.splitMode==="equal"?`・${expense.splitWith.length} 人平均分帳`:"・不分帳"}</span></div><div className="expense-amount"><strong>{expense.currency} {expense.amount.toLocaleString(undefined,{maximumFractionDigits:2})}</strong>{expense.splitMode==="equal"&&expense.splitWith.length>0&&<small>每人 {(expense.amount/expense.splitWith.length).toLocaleString(undefined,{maximumFractionDigits:2})}</small>}</div><div className="expense-actions"><button onClick={()=>openEditExpense(expense)} aria-label={`編輯 ${expense.merchant}`}><Icon name="edit" size={14}/></button><button className="danger" onClick={()=>deleteExpense(expense)} aria-label={`刪除 ${expense.merchant}`}><Icon name="trash" size={14}/></button></div></article>)}</div></>}</section>}
    ...
    {modal==="expense"&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setModal(null)}><form className="upload-modal trip-form expense-form" onSubmit={saveExpense} onMouseDown={event=>event.stopPropagation()}><div className="modal-handle"/><button type="button" className="modal-close" onClick={()=>setModal(null)}><Icon name="close"/></button><span className="modal-icon receipt"><Icon name={editingExpenseId?"edit":"receipt"} size={29}/></span><h2>{editingExpenseId?"編輯帳目":"新增帳目"}</h2><p>可以手動填寫，亦可即時拍照或從相簿選擇收據；核對後才會入帳。</p>{!editingExpenseId&&<><input ref={receiptCameraRef} type="file" accept="image/*" capture="environment" hidden onChange={event=>chooseReceiptFile(event.target.files?.[0]??null)}/><input ref={receiptLibraryRef} type="file" accept="image/*" hidden onChange={event=>chooseReceiptFile(event.target.files?.[0]??null)}/>{receiptPreviewUrl?<button type="button" className="receipt-picker selected" onClick={()=>receiptLibraryRef.current?.click()}><img src={receiptPreviewUrl} alt="待辨認收據"/><span className="replace-receipt">更換相片</span></button>:<div className="receipt-source-actions"><button type="button" onClick={()=>receiptCameraRef.current?.click()}><Icon name="camera"/><strong>即時拍照</strong><small>開啟相機拍攝收據</small></button><button type="button" onClick={()=>receiptLibraryRef.current?.click()}><Icon name="upload"/><strong>從相簿選擇</strong><small>揀選已有收據相片</small></button></div>}{pickedFile&&<button type="button" className="receipt-recognize" disabled={receiptBusy} onClick={()=>void recognizeReceipt()}><Icon name="sparkle" size={16}/>{receiptBusy?`正在辨認 ${receiptProgress}%`:"辨認商戶、日期及總額"}</button>}</>}<div className="form-grid expense-fields"><label><span>商戶／項目 *</span><input value={expenseDraft.merchant} onChange={event=>setExpenseField("merchant",event.target.value)} placeholder="例如：韓國烤肉晚餐" required/></label><label><span>日期 *</span><input type="date" min={activeTrip.startDate} max={activeTrip.endDate} value={expenseDraft.date} onChange={event=>setExpenseField("date",event.target.value)} required/></label><label><span>類別</span><select value={expenseDraft.category} onChange={event=>setExpenseField("category",event.target.value)}><option>餐飲</option><option>交通</option><option>住宿</option><option>購物</option><option>景點</option><option>其他</option></select></label><label><span>貨幣</span><select value={expenseDraft.currency} onChange={event=>setExpenseField("currency",event.target.value)}><option>HKD</option><option>JPY</option><option>KRW</option><option>TWD</option><option>USD</option><option>EUR</option><option>CNY</option></select></label><label><span>金額 *</span><input type="number" min="0.01" step="0.01" inputMode="decimal" value={expenseDraft.amount||""} onChange={event=>setExpenseField("amount",Number(event.target.value))} required/></label><label><span>付款人</span><select value={expenseDraft.paidBy} onChange={event=>setExpenseField("paidBy",event.target.value)}>{expenseParticipants.map(person=><option key={person} value={person}>{person===user?.email?.toLowerCase()?"我":person}</option>)}</select></label><label><span>分帳方式</span><select value={expenseDraft.splitMode} onChange={event=>{const mode=event.target.value as Expense["splitMode"];setExpenseDraft(current=>({...current,splitMode:mode,splitWith:mode==="equal"?(current.splitWith.length?current.splitWith:expenseParticipants):[]}))}}><option value="none">不分帳</option><option value="equal">同行者平均分帳</option></select></label>{expenseDraft.splitMode==="equal"&&<fieldset className="split-people"><legend>參與分帳的人</legend>{expenseParticipants.map(person=><label key={person}><input type="checkbox" checked={expenseDraft.splitWith.includes(person)} onChange={()=>toggleSplitPerson(person)}/><span>{person===user?.email?.toLowerCase()?"我":person}</span></label>)}</fieldset>}<label><span>備註</span><textarea rows={3} value={expenseDraft.note} onChange={event=>setExpenseField("note",event.target.value)} placeholder="例如：晚餐、訂座或退款資料"/></label></div>{receiptRawText&&<details className="ocr-raw"><summary>查看收據原始辨認文字</summary><pre>{receiptRawText}</pre></details>}<button className="confirm-button" type="submit">{editingExpenseId?"儲存更改":"確認入帳"}</button></form></div>}
