...
function PreparationSharedPanel({destination,packingItems,participants,accommodation,showAccommodationCard,onPackingChange,onPackingAdd,onCopy}:{destination:string;packingItems:PackingItem[];participants:string[];accommodation?:HotelDetails;showAccommodationCard:boolean;onPackingChange:(id:string,changes:Partial<PackingItem>)=>void;onPackingAdd:(title:string,assignedTo:string)=>void;onCopy:(label:string,value?:string)=>void}){
...
export default function Home(){
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="準備"&&<section className="screen-content shared-preparation-content"><div className="page-title"><div><p>{activeTrip.name}</p><h2>出發前準備</h2></div><span className="completion-ring" style={{"--progress":`${prepPercent}%`} as React.CSSProperties}>{prepPercent}%</span></div><PreparationSharedPanel destination={activeTrip.destination} packingItems={packingItems} participants={expenseParticipants} accommodation={effectiveHotelDetails} showAccommodationCard={isSharedEditor} onPackingChange={updatePackingItem} onPackingAdd={addPackingItem} onCopy={(label,value)=>void copyText(label,value)}/></section>}
