...
function emptyEntryForm(destination:string):EntryFormDetails{return {platform:isJapanDestination(destination)?"Visit Japan Web":"入境申報",travelerName:"",arrivalFlightNumber:"",purpose:"旅遊",referenceNumber:"",notes:"",updatedBy:""}}

function PreparationSharedPanel({destination,packingItems,participants,accommodation,entryForm,onPackingChange,onPackingAdd,onPackingDelete,onSave}:{destination:string;packingItems:PackingItem[];participants:string[];accommodation?:HotelDetails;entryForm?:EntryFormDetails;onPackingChange:(id:string,changes:Partial<PackingItem>)=>void;onPackingAdd:(title:string,quantity:string,assignedTo:string)=>void;onPackingDelete:(id:string)=>void;onSave:(accommodation:HotelDetails,entryForm:EntryFormDetails)=>void}){
  const [newItem,setNewItem]=useState("");
  const [newQuantity,setNewQuantity]=useState("1 份");
  const [newAssignee,setNewAssignee]=useState("未指定");
  const submitPacking=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const title=newItem.trim();if(!title)return;onPackingAdd(title,newQuantity.trim()||"1 份",newAssignee);setNewItem("");setNewQuantity("1 份")};
  const submitShared=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const data=new FormData(event.currentTarget);onSave({name:String(data.get("hotelName")||"").trim(),address:String(data.get("hotelAddress")||"").trim(),postalCode:String(data.get("postalCode")||"").trim(),phone:String(data.get("hotelPhone")||"").trim(),checkInDate:String(data.get("checkInDate")||""),checkInTime:String(data.get("checkInTime")||""),checkOutDate:String(data.get("checkOutDate")||""),checkOutTime:String(data.get("checkOutTime")||"")},{platform:String(data.get("platform")||"").trim(),travelerName:String(data.get("travelerName")||"").trim(),arrivalFlightNumber:String(data.get("arrivalFlightNumber")||"").trim().toUpperCase(),purpose:String(data.get("purpose")||"").trim(),referenceNumber:String(data.get("referenceNumber")||"").trim(),notes:String(data.get("entryNotes")||"").trim(),updatedBy:String(data.get("updatedBy")||"").trim()})};
  const entry=entryForm??emptyEntryForm(destination);
  return <>
    ...
    <section className="packing-panel">
      <p className="shared-prep-note">任何同行者都可以新增、修改、完成及指定由誰攜帶。</p>
      <div className="packing-list">{packingItems.map(item=><article className={item.done?"done":""} key={item.id}>
        ...
        <div className="packing-fields"><input aria-label="物品名稱" value={item.title} onChange={event=>onPackingChange(item.id,{title:event.target.value})}/><input aria-label="數量" value={item.quantity} onChange={event=>onPackingChange(item.id,{quantity:event.target.value})}/><input aria-label="物品備註" value={item.note} onChange={event=>onPackingChange(item.id,{note:event.target.value})} placeholder="備註（選填）"/></div>
        <label className="packing-assignee"><span>由誰帶</span><select value={item.assignedTo} onChange={event=>onPackingChange(item.id,{assignedTo:event.target.value})}>{["未指定","共同攜帶",...participants].filter((value,index,list)=>list.indexOf(value)===index).map(person=><option key={person}>{person}</option>)}</select></label>
      ...
      <form className="packing-add" onSubmit={submitPacking}><input value={newItem} onChange={event=>setNewItem(event.target.value)} placeholder="新增物品，例如：Wi-Fi 蛋" aria-label="新增物品"/><input value={newQuantity} onChange={event=>setNewQuantity(event.target.value)} placeholder="數量" aria-label="新增物品數量"/><select value={newAssignee} onChange={event=>setNewAssignee(event.target.value)} aria-label="指定攜帶者">{["未指定","共同攜帶",...participants].filter((value,index,list)=>list.indexOf(value)===index).map(person=><option key={person}>{person}</option>)}</select><button type="submit" disabled={!newItem.trim()}><Icon name="plus" size={16}/>加入</button></form>
    </section>
    <div className="shared-prep-heading"><div><small>同行共同提供</small><h3>{isJapanDestination(destination)?"住宿及 Visit Japan 填表資料":"住宿及入境填表資料"}</h3></div><span><Icon name="shield" size={16}/>原圖不共享</span></div>
    <form className="shared-travel-form" key={`${destination}-${JSON.stringify(accommodation)}-${JSON.stringify(entryForm)}`} onSubmit={submitShared}>
      <p className="shared-prep-note">以下文字資料會與同行者共享；酒店／機票原圖仍只限旅程擁有人查看。護照號碼等敏感資料請各自保管。</p>
      <fieldset><legend>住宿資料</legend><div className="shared-form-grid"><label><span>酒店名稱</span><input name="hotelName" defaultValue={accommodation?.name??""} placeholder="例如：首爾明洞酒店"/></label><label><span>郵遞區號</span><input name="postalCode" defaultValue={accommodation?.postalCode??""}/></label><label className="wide"><span>酒店地址</span><textarea name="hotelAddress" rows={2} defaultValue={accommodation?.address??""}/></label><label><span>電話號碼</span><input name="hotelPhone" type="tel" defaultValue={accommodation?.phone??""}/></label><label><span>入住日期</span><input name="checkInDate" type="date" defaultValue={accommodation?.checkInDate??""}/></label><label><span>入住時間</span><input name="checkInTime" type="time" defaultValue={accommodation?.checkInTime??""}/></label><label><span>退房日期</span><input name="checkOutDate" type="date" defaultValue={accommodation?.checkOutDate??""}/></label><label><span>退房時間</span><input name="checkOutTime" type="time" defaultValue={accommodation?.checkOutTime??""}/></label></div></fieldset>
      <fieldset><legend>{isJapanDestination(destination)?"Visit Japan Web 資料":"入境填表資料"}</legend><div className="shared-form-grid"><label><span>申報平台</span><input name="platform" defaultValue={entry.platform}/></label><label><span>旅客姓名</span><input name="travelerName" defaultValue={entry.travelerName}/></label><label><span>抵達航班</span><input name="arrivalFlightNumber" defaultValue={entry.arrivalFlightNumber} placeholder="例如：KE602"/></label><label><span>入境目的</span><input name="purpose" defaultValue={entry.purpose}/></label><label><span>登記／QR 參考編號</span><input name="referenceNumber" defaultValue={entry.referenceNumber}/></label><label><span>最後更新者</span><select name="updatedBy" defaultValue={entry.updatedBy}><option value="">未指定</option>{participants.map(person=><option key={person}>{person}</option>)}</select></label><label className="wide"><span>入境備註</span><textarea name="entryNotes" rows={3} defaultValue={entry.notes} placeholder="例如：已完成申報、尚欠同行者資料"/></label></div></fieldset>
      <button className="shared-prep-save" type="submit"><Icon name="check" size={16}/>儲存並同步給同行者</button>
    </form>
  </>
}
...
export default function Home(){
  ...
  const addPackingItem=(title:string,quantity:string,assignedTo:string)=>{setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,packingItems:[...(trip.packingItems??packingTemplates()),{id:crypto.randomUUID(),title,quantity,assignedTo,done:false,note:""}]}:trip));toast("物品已加入共同清單")};
  const deletePackingItem=(id:string)=>{setTrips(current=>current.map(trip=>{if(trip.id!==activeTrip.id)return trip;const list=trip.packingItems??packingTemplates();return {...trip,packingItems:list.filter(item=>item.id!==id)}}));toast("物品已刪除")};
  const saveSharedPreparation=(accommodation:HotelDetails,entryForm:EntryFormDetails)=>{setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,sharedAccommodation:accommodation,entryFormDetails:entryForm}:trip));toast("住宿及入境資料已儲存並準備同步")};
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="準備"&&<section className="screen-content shared-preparation-content"><div className="page-title"><div><p>{activeTrip.name}</p><h2>出發前準備</h2></div><span className="completion-ring" style={{"--progress":`${prepPercent}%`} as React.CSSProperties}>{prepPercent}%</span></div><PreparationSharedPanel destination={activeTrip.destination} packingItems={packingItems} participants={expenseParticipants} accommodation={effectiveHotelDetails} entryForm={activeTrip.entryFormDetails} onPackingChange={updatePackingItem} onPackingAdd={addPackingItem} onPackingDelete={deletePackingItem} onSave={saveSharedPreparation}/></section>}
