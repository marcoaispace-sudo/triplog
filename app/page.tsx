...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    const loadCloud=async()=>{
      ...
      const cloudTrips=((data??[]) as TripRow[]).map(row=>tripFromRow(row));
      ...
      if(shouldMigrate||!cloudTrips.length){
        ...
        const ownedTrips=nextTrips.filter(trip=>trip.accessRole!=="editor");
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(async()=>{
      setCloudState("connecting");
      const ownedTrips=trips.filter(trip=>trip.accessRole!=="editor");
  ...
  const openCompanionManager=(tripId=activeTrip.id)=>{setActiveTripId(tripId);setCollaboratorEmail("");setCollaboratorMessage("");setModal("companion");void loadCollaborators(tripId)};
  const inviteCollaborator=async(event:FormEvent<HTMLFormElement>)=>{
    ...
    const redirectTo=`${window.location.origin}${basePath}/?trip_invite=${encodeURIComponent(String(token))}`;
    const {error:mailError}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:redirectTo,shouldCreateUser:true}});
    setCollaboratorBusy(false);
    if(mailError){setCollaboratorMessage(`邀請已建立，但電郵未能寄出：${mailError.message}`);return}
    setCollaboratorEmail("");setCollaboratorMessage("邀請電郵已寄出；連結 7 日內有效");
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="首頁"&&<>
      ...
      <section className="day-section"><div className="section-heading day-heading"><div><p>第{selectedDayNumber}日安排</p><h3>{activeTrip.destination}行程</h3></div><button className="recommend-button" onClick={()=>void openNearbyRecommendations()}><Icon name="pin" size={16}/>附近推介</button></div><Timeline items={dayItems} onMap={openMap}/></section>
    </>}
