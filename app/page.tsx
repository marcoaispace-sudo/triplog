...
type ModalType = "ticket"|"hotel"|"receipt"|"trip"|"itinerary"|"ocr"|null;
...
function tripFromRow(row:TripRow):Trip{
  ...
  return {id:row.id,name:row.name,destination:row.destination_city??"未設定",startDate:row.start_date??new Date().toISOString().slice(0,10),endDate:row.end_date??row.start_date??new Date().toISOString().slice(0,10),origin:stored.origin??"HKG",destinationAirport:stored.destinationAirport??"---",companions:Number(stored.companions??1),budget:Number(stored.budget??0),state:row.deleted_at?"deleted":row.status==="archived"?"archived":"active",createdAt:row.created_at,deletedAt:row.deleted_at??undefined,itinerary:Array.isArray(stored.itinerary)?stored.itinerary:[],requirements:Array.isArray(stored.requirements)?stored.requirements:undefined,documents:Array.isArray(stored.documents)?stored.documents:[],hotelDetails:stored.hotelDetails};
}
...
export default function Home(){
  ...
  const pick=(type:ModalType)=>{setPicked("");setPickedFile(null);setModal(type)};
  ...
  const recognizeDocument=async(document:TripDocument)=>{
    ...
    setOcrDocument(document);setOcrBusy(true);setOcrProgress(0);setOcrStatus("準備下載私人相片…");setOcrFields(document.ocr??emptyOcrFields);setOcrRawText("");setOcrError("");setModal("ocr");
    ...
    try{
      ...
      setOcrRawText(result.data.text);setOcrFields(fields);setOcrProgress(100);setOcrStatus("辨認完成，請核對以下資料");
  ...
  const confirmOcr=(event:FormEvent<HTMLFormElement>)=>{
    ...
    setTrips(current=>current.map(trip=>{if(trip.id!==activeTrip.id)return trip;const documents=(trip.documents??[]).map(document=>document.id===ocrDocument.id?{...document,ocr:ocrFields,recognizedAt}:document);if(ocrDocument.kind==="ticket"){const startDate=ocrFields.departureDate||trip.startDate;return {...trip,documents,origin:ocrFields.origin.toUpperCase()||trip.origin,destinationAirport:ocrFields.destinationAirport.toUpperCase()||trip.destinationAirport,startDate,endDate:trip.endDate<startDate?startDate:trip.endDate}}return {...trip,documents,hotelDetails:{name:ocrFields.hotelName,address:ocrFields.address,postalCode:ocrFields.postalCode,phone:ocrFields.phone}}}));
    setModal(null);setOcrDocument(null);toast(ocrDocument.kind==="ticket"?"已填入航班資料並準備同步":"已保存酒店及 Visit Japan 所需資料");
  };
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {modal==="ocr"&&ocrDocument&&<div className="modal-backdrop" role="presentation"><form className="upload-modal trip-form ocr-modal" onSubmit={confirmOcr} onMouseDown={e=>e.stopPropagation()}><div className="modal-handle"/><button type="button" className="modal-close" disabled={ocrBusy} onClick={()=>setModal(null)}><Icon name="close"/></button><span className={`modal-icon ${ocrDocument.kind}`}><Icon name="sparkle" size={29}/></span><h2>{ocrDocument.kind==="ticket"?"辨認機票資料":"辨認酒店資料"}</h2><p>{ocrDocument.name}・相片文字會在你目前裝置上辨認。</p>{ocrBusy&&<div className="ocr-working" role="status"><div className="ocr-progress"><span style={{width:`${ocrProgress}%`}}/></div><strong>{ocrStatus}</strong><small>{ocrProgress}%・首次使用需要下載語言資料，請勿關閉。</small></div>}{ocrError&&<div className="ocr-error" role="alert"><strong>未能完成辨認</strong><span>{ocrError}</span><button type="button" onClick={()=>void recognizeDocument(ocrDocument)}>再試一次</button></div>}{!ocrBusy&&!ocrError&&<><div className="ocr-warning"><Icon name="shield" size={17}/><span>免費 OCR 可能認錯，儲存前請逐項核對。</span></div><div className="form-grid ocr-form">{ocrDocument.kind==="ticket"?<><label><span>航班編號</span><input value={ocrFields.flightNumber} onChange={e=>setOcrField("flightNumber",e.target.value.toUpperCase())} placeholder="例如 CX500"/></label><label><span>出發日期</span><input type="date" value={ocrFields.departureDate} onChange={e=>setOcrField("departureDate",e.target.value)}/></label><label><span>出發機場</span><input maxLength={3} value={ocrFields.origin} onChange={e=>setOcrField("origin",e.target.value.toUpperCase())} placeholder="HKG"/></label><label><span>抵達機場</span><input maxLength={3} value={ocrFields.destinationAirport} onChange={e=>setOcrField("destinationAirport",e.target.value.toUpperCase())} placeholder="NRT"/></label></>:<><label><span>酒店名稱</span><input value={ocrFields.hotelName} onChange={e=>setOcrField("hotelName",e.target.value)} placeholder="酒店名稱"/></label><label><span>郵遞區號</span><input value={ocrFields.postalCode} onChange={e=>setOcrField("postalCode",e.target.value)} placeholder="例如 160-0023"/></label><label><span>酒店地址</span><textarea rows={3} value={ocrFields.address} onChange={e=>setOcrField("address",e.target.value)} placeholder="Visit Japan 所需地址"/></label><label><span>電話號碼</span><input value={ocrFields.phone} onChange={e=>setOcrField("phone",e.target.value)} placeholder="酒店電話"/></label></>}</div>{ocrRawText&&<details className="ocr-raw"><summary>查看原始辨認文字</summary><pre>{ocrRawText}</pre></details>}<button className="confirm-button" type="submit">確認並填入旅程</button></>}</form></div>}
