...
import { loadDailyWeather, loadDailyWeatherAtCoordinates, weatherDescription, type DailyWeather } from "./weather";
...
type ModalType = "ticket"|"hotel"|"receipt"|"expense"|"expenseView"|"trip"|"itinerary"|"ocr"|"recommend"|"companion"|"quickInfo"|null;
type TripState = "active"|"archived"|"deleted";
type Screen = "首頁"|"旅程"|"準備"|"行程"|"記帳"|"設定"|"回顧";
...
type PackingItem = {id:string;title:string;quantity:string;assignedTo:string;done:boolean;note:string};
...
type EntryFormDetails = {platform:string;travelerName:string;arrivalFlightNumber:string;purpose:string;referenceNumber:string;notes:string;updatedBy:string};
type TravelQuickInfo = {insuranceProvider:string;policyNumber:string;insuranceHotline:string;emergencyContactName:string;emergencyContactPhone:string};
...
type Trip = {id:string;name:string;destination:string;startDate:string;endDate:string;origin:string;destinationAirport:string;companions:number;budget:number;state:TripState;createdAt:string;deletedAt?:string;itinerary?:ItineraryItem[];requirements?:Requirement[];expenses?:Expense[];packingItems?:PackingItem[];sharedAccommodation?:HotelDetails;entryFormDetails?:EntryFormDetails;quickInfo?:TravelQuickInfo;documents?:TripDocument[];hotelDetails?:HotelDetails;accessRole?:TripAccess};
...
type MapChoice = {mode:"place";item:ItineraryItem}|{mode:"directions";from:ItineraryItem;to:ItineraryItem}|{mode:"current";to:ItineraryItem;destinationLabel:"酒店"|"機場"};
...
function sharedLedgerPayload(trip:Trip){return {items:trip.requirements??[],expenses:trip.expenses??[],packingItems:trip.packingItems??packingTemplates(),accommodation:trip.sharedAccommodation,entryForm:trip.entryFormDetails,quickInfo:trip.quickInfo}}
...
function tripFromRow(row:TripRow):Trip{
  let meta:{triplog?:{origin?:string;destinationAirport?:string;companions?:number;budget?:number;itinerary?:ItineraryItem[];requirements?:Requirement[]|{items?:Requirement[];expenses?:Expense[];packingItems?:PackingItem[];accommodation?:HotelDetails;entryForm?:EntryFormDetails;quickInfo?:TravelQuickInfo};expenses?:Expense[];documents?:TripDocument[];hotelDetails?:HotelDetails}}={};
  ...
  return {id:row.id,name:row.name,destination:row.destination_city??"未設定",startDate:row.start_date??localTodayDateOnly(),endDate:row.end_date??row.start_date??localTodayDateOnly(),origin:stored.origin??"HKG",destinationAirport:stored.destinationAirport??"---",companions:Number(stored.companions??1),budget:Number(stored.budget??0),state:row.deleted_at?"deleted":row.status==="archived"?"archived":"active",createdAt:row.created_at,deletedAt:row.deleted_at??undefined,itinerary:Array.isArray(stored.itinerary)?stored.itinerary:[],requirements:Array.isArray(stored.requirements)?stored.requirements:Array.isArray(requirementEnvelope?.items)?requirementEnvelope.items:undefined,expenses:Array.isArray(stored.expenses)?stored.expenses:Array.isArray(requirementEnvelope?.expenses)?requirementEnvelope.expenses:[],packingItems:Array.isArray(requirementEnvelope?.packingItems)?requirementEnvelope.packingItems:undefined,sharedAccommodation:requirementEnvelope?.accommodation,entryFormDetails:requirementEnvelope?.entryForm,quickInfo:requirementEnvelope?.quickInfo,documents:Array.isArray(stored.documents)?stored.documents:[],hotelDetails:stored.hotelDetails,accessRole:row.access_role??"owner"};
}
...
function isKoreaDestination(destination:string){return /韓國|南韓|首爾|釜山|仁川|濟州|大邱|seoul|busan|incheon|jeju|korea/i.test(destination)}
...
function packingTemplates():PackingItem[]{return [
  {id:"pack-passport",title:"護照及入境文件",quantity:"每人 1 份",assignedTo:"未指定",done:false,note:""},
  {id:"pack-charger",title:"手機充電器／旅行轉插",quantity:"1 套",assignedTo:"未指定",done:false,note:""},
  {id:"pack-medicine",title:"常用藥物",quantity:"按需要",assignedTo:"未指定",done:false,note:""},
  {id:"pack-rain",title:"雨具",quantity:"1 份",assignedTo:"未指定",done:false,note:""},
]}
function PreparationSharedPanel({destination,packingItems,participants,accommodation,showAccommodationCard,onPackingChange,onPackingAdd,onPackingDelete,onCopy}:{destination:string;packingItems:PackingItem[];participants:string[];accommodation?:HotelDetails;showAccommodationCard:boolean;onPackingChange:(id:string,changes:Partial<PackingItem>)=>void;onPackingAdd:(title:string,assignedTo:string)=>void;onPackingDelete:(id:string)=>void;onCopy:(label:string,value?:string)=>void}){
  const [newItem,setNewItem]=useState("");
  const [newAssignee,setNewAssignee]=useState("未指定");
  const submitPacking=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const title=newItem.trim();if(!title)return;onPackingAdd(title,newAssignee);setNewItem("")};
  return <>
    <div className="shared-prep-heading"><div><small>同行共同編輯</small><h3>攜帶物品清單</h3></div><span><Icon name="users" size={16}/>雙方同步</span></div>
    <section className="packing-panel">
      <p className="shared-prep-note">上方是共同清單；在下方輸入物品及選擇邊個帶，再按加入。</p>
      <div className="packing-list">{packingItems.map(item=><article className={item.done?"done":""} key={item.id}>
        <button type="button" className="packing-check" aria-label={`${item.done?"取消完成":"標記完成"} ${item.title}`} onClick={()=>onPackingChange(item.id,{done:!item.done})}><Icon name="check" size={15}/></button>
        <div className="packing-copy"><strong>{item.title}</strong><span>{item.assignedTo||"未指定"}</span></div>
        <button type="button" className="packing-delete" aria-label={`刪除 ${item.title}`} onClick={()=>onPackingDelete(item.id)}><Icon name="trash" size={15}/></button>
      </article>)}</div>
      <form className="packing-add" onSubmit={submitPacking}><label><span>物品</span><input value={newItem} onChange={event=>setNewItem(event.target.value)} placeholder="例如：Wi-Fi 蛋" aria-label="新增物品"/></label><label><span>邊個帶</span><select value={newAssignee} onChange={event=>setNewAssignee(event.target.value)} aria-label="指定攜帶者">{["未指定","共同攜帶",...participants].filter((value,index,list)=>list.indexOf(value)===index).map(person=><option key={person}>{person}</option>)}</select></label><button type="submit" disabled={!newItem.trim()}><Icon name="plus" size={16}/>加入</button></form>
    </section>
    {showAccommodationCard&&<><div className="shared-prep-heading"><div><small>供同行者填寫入境表</small><h3>{isJapanDestination(destination)?"住宿及 Visit Japan 填表資料":"住宿及入境填表資料"}</h3></div><span><Icon name="shield" size={16}/>只供查看</span></div>{accommodation&&Object.values(accommodation).some(Boolean)?<article className="hotel-info-card companion-hotel-card"><header><span><Icon name="hotel" size={19}/></span><div><small>住宿資料</small><strong>{accommodation.name||"酒店名稱尚未提供"}</strong></div></header><div className="hotel-stay-dates"><span><small>入住</small><strong>{accommodation.checkInDate?dateLabel(accommodation.checkInDate):"待提供"} {accommodation.checkInTime||""}</strong></span><span><small>退房</small><strong>{accommodation.checkOutDate?dateLabel(accommodation.checkOutDate):"待提供"} {accommodation.checkOutTime||""}</strong></span></div>{[["郵遞區號",accommodation.postalCode],["酒店地址",accommodation.address],["電話號碼",accommodation.phone]].map(([label,value])=><div className="hotel-copy-row" key={label}><span><small>{label}</small><strong>{value||"尚未提供"}</strong></span><button type="button" onClick={()=>onCopy(label,value)} disabled={!value}>複製</button></div>)}<p className="companion-hotel-note">資料由旅程擁有人提供，只供同行者填寫入境表；酒店文件原圖不會共享。</p></article>:<div className="companion-hotel-empty"><Icon name="hotel" size={18}/><div><strong>住宿資料尚未提供</strong><span>旅程擁有人更新酒店資料後，會自動同步到這裡。</span></div></div>}</>}
  </>
}

function JourneyReview({trip,onBack,onOpenDocument,onOpenExpense}:{trip:Trip;onBack:()=>void;onOpenDocument:(document:TripDocument)=>void;onOpenExpense:(expense:Expense)=>void}){
  const itinerary=(trip.itinerary??[]).filter(item=>item.plan!=="backup").sort((a,b)=>`${a.date} ${a.time||"99:99"}`.localeCompare(`${b.date} ${b.time||"99:99"}`));
  const expenses=(trip.expenses??[]).slice().sort((a,b)=>b.date.localeCompare(a.date)||b.createdAt.localeCompare(a.createdAt));
  const total=expenses.reduce((sum,item)=>sum+expenseHkdAmount(item),0);
  const visitedPlaces=new Set(itinerary.filter(item=>item.location||["景點","餐廳","餐飲","住宿"].includes(item.type)).map(item=>(item.location||item.title).trim().toLowerCase()).filter(Boolean)).size;
  const receiptCount=expenses.filter(item=>item.receiptPath).length;
  return <section className="screen-content journey-review"><div className="review-topbar"><button onClick={onBack}><Icon name="arrow" size={17}/>返回</button><span>旅程回顧</span></div>
    <article className="review-hero"><span className="review-kicker">一段值得重溫的旅程</span><h2>{trip.name}</h2><p>{dateLabel(trip.startDate)} — {dateLabel(trip.endDate)}・{tripDuration(trip)}日</p><div className="review-route"><strong>{trip.origin}</strong><span><Icon name="plane" size={17}/></span><strong>{trip.destinationAirport}</strong></div><div className="review-hero-meta"><span><Icon name="pin" size={15}/>{trip.destination}</span><span><Icon name="users" size={15}/>{trip.companions} 位旅客</span></div></article>
    <div className="review-stats"><article><strong>{tripDuration(trip)}</strong><span>旅程日數</span></article><article><strong>{itinerary.length}</strong><span>行程安排</span></article><article><strong>{visitedPlaces}</strong><span>去過地點</span></article><article><strong>{expenses.length}</strong><span>開支記錄</span></article></div>
    <div className="review-section-heading"><div><p>每日足跡</p><h3>行程時間線</h3></div></div>
    <div className="review-days">{tripDates(trip).map((date,index)=>{const items=itinerary.filter(item=>item.date===date);return <article className="review-day" key={date}><header><span>第 {index+1} 日</span><div><strong>{dateLabel(date)}</strong><small>{weekdayLabel(date)}・{items.length} 項安排</small></div></header><Timeline items={items}/></article>})}</div>
    <div className="review-section-heading"><div><p>全程以港幣計算</p><h3>開支回顧</h3></div><strong>HKD {total.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong></div>
    {expenses.length?<><ExpensePieChart items={expenses}/><div className="review-expenses">{expenses.map(expense=><button key={expense.id} onClick={()=>onOpenExpense(expense)}><span><small>{dateLabel(expense.date)}・{expense.category}</small><strong>{expense.merchant}</strong></span><em>HKD {expenseHkdAmount(expense).toLocaleString(undefined,{maximumFractionDigits:2})}</em><Icon name="eye" size={14}/></button>)}</div></>:<div className="review-empty"><Icon name="receipt"/><span><strong>未有開支記錄</strong><small>今次旅程沒有已儲存帳目。</small></span></div>}
    <div className="review-section-heading"><div><p>機票、酒店及收據</p><h3>旅程記錄</h3></div><span>{(trip.documents??[]).length+receiptCount} 份</span></div>
    {(trip.documents??[]).length||receiptCount?<div className="review-documents">{(trip.documents??[]).map(document=><button key={document.id} onClick={()=>onOpenDocument(document)} disabled={trip.accessRole==="editor"}><span><Icon name={document.kind==="ticket"?"plane":"hotel"} size={18}/></span><div><strong>{document.name}</strong><small>{document.kind==="ticket"?"機票":"酒店"}{trip.accessRole==="editor"?"・私人文件":"・按此查看"}</small></div><Icon name="eye" size={15}/></button>)}{receiptCount>0&&<article><span><Icon name="receipt" size={18}/></span><div><strong>{receiptCount} 張收據相片</strong><small>可於上方開支記錄逐筆查看</small></div></article>}</div>:<div className="review-empty"><Icon name="archive"/><span><strong>未有旅程文件</strong><small>今次旅程沒有已儲存文件或收據。</small></span></div>}
  </section>
}

function TripManager({trips,activeTripId,cloudState,isSignedIn,onSelect,onAdd,onArchive,onDelete,onRestore,onCompanions,onReview}:{trips:Trip[];activeTripId:string;cloudState:CloudState;isSignedIn:boolean;onSelect:(id:string)=>void;onAdd:()=>void;onArchive:(id:string)=>void;onDelete:(id:string)=>void;onRestore:(id:string)=>void;onCompanions:(id:string)=>void;onReview:(id:string)=>void}){
...
export default function Home(){
  ...
  const [selectedDate,setSelectedDate]=useState<{tripId:string;date:string}|null>(null);
  ...
  const reviewTrip=useMemo(()=>trips.find(t=>t.id===reviewTripId)??activeTrip,[trips,reviewTripId,activeTrip]);
  const dates=useMemo(()=>tripDates(activeTrip),[activeTrip]);
  const automaticTripDate=homeScheduleDate(activeTrip.startDate,activeTrip.endDate,[],today);
  const shownDate=selectedDate?.tripId===activeTrip.id&&dates.includes(selectedDate.date)?selectedDate.date:automaticTripDate;
  const effectiveHotelDetails=activeTrip.sharedAccommodation??activeTrip.hotelDetails;
  const weatherQueries=useMemo(()=>[effectiveHotelDetails?.address??"",effectiveHotelDetails?.name?`${effectiveHotelDetails.name} ${activeTrip.destination}`:"",activeTrip.destination],[activeTrip.destination,effectiveHotelDetails?.address,effectiveHotelDetails?.name]);
  useEffect(()=>{
    const controller=new AbortController();
    queueMicrotask(()=>{if(!controller.signal.aborted){setWeatherState("loading");setWeatherDays({});setWeatherLocation("");setWeatherMessage("")}});
    const load=async()=>{
      const hotelName=effectiveHotelDetails?.name?.trim();
      if(GOOGLE_MAPS_KEY&&hotelName){
        try{
          const coordinates=await findPlaceCoordinates(`${hotelName} ${activeTrip.destination}`,GOOGLE_MAPS_KEY);
          if(coordinates)return loadDailyWeatherAtCoordinates(coordinates.lat,coordinates.lng,hotelName,controller.signal);
        }catch{/* Google 酒店定位失敗時改用城市名稱後備 */}
      }
      return loadDailyWeather(weatherQueries,controller.signal);
    };
    void load().then(({location,days})=>{
    ...
    }).catch(error=>{if(!controller.signal.aborted){setWeatherMessage(error instanceof Error&&error.message==="location not found"?"未能辨認酒店或城市位置":"天氣服務暫時未能連線");setWeatherState("error")}});
    return()=>controller.abort();
  },[activeTrip.destination,effectiveHotelDetails?.name,weatherQueries]);
  ...
  const hotelRouteOrigin=useMemo<ItineraryItem|undefined>(()=>{
    const name=effectiveHotelDetails?.name?.trim();
    const address=effectiveHotelDetails?.address?.trim();
  ...
  },[activeTrip.destination,effectiveHotelDetails?.address,effectiveHotelDetails?.name,activeTrip.id]);
  const quickFlightItems=useMemo(()=>(activeTrip.itinerary??[]).filter(item=>item.plan!=="backup"&&item.type==="交通"&&/航班|出發|抵達|機場|flight/i.test(`${item.title} ${item.note}`)).sort((a,b)=>`${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`)).slice(0,4),[activeTrip.itinerary]);
  ...
  const packingItems=useMemo(()=>activeTrip.packingItems??packingTemplates(),[activeTrip.packingItems]);
  ...
  useEffect(()=>{
    if(active!=="準備"||!supabase||!user||isSharedEditor)return;
    void supabase.rpc("triplog_list_collaborators",{p_trip_id:activeTrip.id}).then(({data})=>setCollaborators((data??[]) as Collaborator[]));
  },[active,activeTrip.id,isSharedEditor,user?.id]);
  ...
  const openReturnDirections=()=>{
    const isLastDay=shownDate===activeTrip.endDate;
    if(isLastDay){
      const airport=activeTrip.destinationAirport.trim();
      if(!airport||airport==="---"){toast("請先在旅程資料補上回程出發機場");return}
      const airportItem:ItineraryItem={id:`${activeTrip.id}-return-airport`,date:shownDate,time:"",title:`${airport} 機場`,note:"",type:"交通",color:"gold",location:`${airport} Airport ${activeTrip.destination}`};
      setMapChoiceMessage("");setMapChoice({mode:"current",to:airportItem,destinationLabel:"機場"});return;
    }
    if(!hotelRouteOrigin){toast("請先在酒店資料補上酒店名稱或地址");return}
    setMapChoiceMessage("");setMapChoice({mode:"current",to:hotelRouteOrigin,destinationLabel:"酒店"});
  };
  const launchMap=async(provider:"google"|"naver")=>{
    ...
    if(provider==="google"){
      const url=mapChoice.mode==="place"
        ...
        :mapChoice.mode==="current"
          ?`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(queryFor(mapChoice.to))}`
    ...
    setMapChoiceMessage(mapChoice.mode==="current"?`正在取得目前位置及定位${mapChoice.destinationLabel}…`:"正在定位上一站及目前行程…");
    try{
      const destinationName=queryFor(mapChoice.to);
      const currentPosition=()=>new Promise<{lat:number;lng:number}>((resolve,reject)=>{
        if(!navigator.geolocation){reject(new Error("此裝置未提供定位功能"));return}
        navigator.geolocation.getCurrentPosition(position=>resolve({lat:position.coords.latitude,lng:position.coords.longitude}),()=>reject(new Error("未能取得目前位置，請允許 TripLog 使用定位")),{enableHighAccuracy:true,timeout:12000,maximumAge:60000});
      });
      const originName=mapChoice.mode==="current"?"目前位置":queryFor(mapChoice.from);
      const [origin,destination]=await Promise.all([
        mapChoice.mode==="current"?currentPosition():findPlaceCoordinates(originName,GOOGLE_MAPS_KEY),
  ...
  const updatePackingItem=(id:string,changes:Partial<PackingItem>)=>setTrips(current=>current.map(trip=>{if(trip.id!==activeTrip.id)return trip;const list=trip.packingItems??packingTemplates();return {...trip,packingItems:list.map(item=>item.id===id?{...item,...changes}:item)}}));
  const addPackingItem=(title:string,assignedTo:string)=>{setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,packingItems:[...(trip.packingItems??packingTemplates()),{id:crypto.randomUUID(),title,quantity:"1 份",assignedTo,done:false,note:""}]}:trip));toast("物品已加入共同清單")};
  const deletePackingItem=(id:string)=>{const item=packingItems.find(entry=>entry.id===id);if(!item||!window.confirm(`確定刪除「${item.title}」？`))return;setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,packingItems:(trip.packingItems??packingTemplates()).filter(entry=>entry.id!==id)}:trip));toast("物品已從共同清單刪除")};
  const saveTravelQuickInfo=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const data=new FormData(event.currentTarget);const quickInfo:TravelQuickInfo={insuranceProvider:String(data.get("insuranceProvider")||"").trim(),policyNumber:String(data.get("policyNumber")||"").trim(),insuranceHotline:String(data.get("insuranceHotline")||"").trim(),emergencyContactName:String(data.get("emergencyContactName")||"").trim(),emergencyContactPhone:String(data.get("emergencyContactPhone")||"").trim()};setTrips(current=>current.map(trip=>trip.id===activeTrip.id?{...trip,quickInfo}:trip));setModal(null);toast("快速資料已儲存並準備同步")};
  ...
  const confirmOcr=(event:FormEvent<HTMLFormElement>)=>{
    ...
    setTrips(current=>current.map(trip=>{if(trip.id!==activeTrip.id)return trip;const documents=(trip.documents??[]).map(document=>document.id===ocrDocument.id?{...document,ocr:ocrFields,recognizedAt}:document);if(ocrDocument.kind==="ticket"){const origin=ocrFields.origin.toUpperCase()||trip.origin;const destinationAirport=ocrFields.destinationAirport.toUpperCase()||trip.destinationAirport;const startDate=ocrFields.departureDate||trip.startDate;const proposedEnd=ocrFields.returnArrivalDate||ocrFields.returnDepartureDate||trip.endDate;const endDate=proposedEnd<startDate?startDate:proposedEnd;const flightIds=`${ocrDocument.id}-flight-`;const itinerary=[...(trip.itinerary??[]).filter(item=>!item.id.startsWith(flightIds)),...flightItineraryItems(ocrDocument.id,ocrFields,origin,destinationAirport)];return {...trip,documents,origin,destinationAirport,startDate,endDate,itinerary}}const hotelIds=`${ocrDocument.id}-hotel-`;const itinerary=[...(trip.itinerary??[]).filter(item=>!item.id.startsWith(hotelIds)),...hotelItineraryItems(ocrDocument.id,ocrFields)];const hotelDetails={name:ocrFields.hotelName,address:ocrFields.address,postalCode:ocrFields.postalCode,phone:ocrFields.phone,checkInDate:ocrFields.checkInDate,checkInTime:ocrFields.checkInTime,checkOutDate:ocrFields.checkOutDate,checkOutTime:ocrFields.checkOutTime};return {...trip,documents,itinerary,hotelDetails,sharedAccommodation:hotelDetails}}));
    setSelectedDate({tripId:activeTrip.id,date:ocrDocument.kind==="ticket"?(ocrFields.departureDate||activeTrip.startDate):(ocrFields.checkInDate||activeTrip.startDate)});setModal(null);setOcrDocument(null);if(ocrImageUrl)URL.revokeObjectURL(ocrImageUrl);setOcrImageUrl("");toast(ocrDocument.kind==="ticket"?"去程及回程時間已加入每日行程":"酒店資料及入住／退房時間已加入旅程");
  };
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="首頁"&&<>
      <section className="greeting"><div><p>晚上好，Man Tat</p><h2>{today>activeTrip.endDate?`重溫 ${activeTrip.destination}。`:`下一站，${activeTrip.destination}。`}</h2></div><button className="all-trips-button" onClick={()=>setActive("旅程")}>全部旅程 <span>{trips.filter(t=>t.state==="active").length}</span></button></section>
      <button className="trip-card trip-card-button" onClick={()=>today>activeTrip.endDate?openJourneyReview(activeTrip.id):setActive("旅程")}><div className="trip-card-top"><div><span className="trip-status">{today>activeTrip.endDate?"旅程已完成":today>=activeTrip.startDate?"旅程進行中":"即將出發"}</span><h3>{activeTrip.name}</h3><p>{dateLabel(activeTrip.startDate)} — {dateLabel(activeTrip.endDate)} · {tripDuration(activeTrip)}日</p></div><div className="countdown"><strong>{today>activeTrip.endDate?tripDuration(activeTrip):Math.max(0,countdown(activeTrip))}</strong><span>{today>activeTrip.endDate?"天旅程":today>=activeTrip.startDate?"旅途中":"日後"}</span></div></div><div className="route-line"><span className="airport">{activeTrip.origin}</span><span className="flight-line"><Icon name="plane" size={17}/></span><span className="airport">{activeTrip.destinationAirport}</span></div><div className="trip-meta"><span><Icon name="users" size={17}/>{activeTrip.companions} 位旅客</span><span>已記錄 <strong>{expenses.length} 筆開支</strong></span></div></button>
      {today>activeTrip.endDate&&<button className="home-review-button" onClick={()=>openJourneyReview(activeTrip.id)}><span><Icon name="eye" size={20}/></span><div><strong>查看旅程回顧</strong><small>每日足跡・全部開支・旅程文件</small></div><Icon name="arrow" size={18}/></button>}
      ...
      <button className="travel-quick-access" onClick={()=>setModal("quickInfo")}><span className="travel-quick-icon"><Icon name="shield" size={21}/></span><span><strong>旅途中快速資料</strong><small>酒店・航班・保險・緊急電話</small></span><em>離線可看</em><Icon name="arrow" size={18}/></button>
    ...
    {active==="旅程"&&<TripManager trips={trips} activeTripId={activeTrip.id} cloudState={cloudState} isSignedIn={Boolean(user)} onSelect={selectTrip} onAdd={()=>setModal("trip")} onArchive={id=>updateTripState(id,"archived")} onDelete={id=>updateTripState(id,"deleted")} onRestore={id=>updateTripState(id,"active")} onCompanions={openCompanionManager} onReview={openJourneyReview}/>}
    {active==="回顧"&&<JourneyReview trip={reviewTrip} onBack={()=>setActive(reviewTrip.state==="archived"?"旅程":"首頁")} onOpenDocument={document=>void openDocument(document)} onOpenExpense={openViewExpense}/>}
    {active==="準備"&&<section className="screen-content shared-preparation-content"><div className="page-title"><div><p>{activeTrip.name}</p><h2>出發前準備</h2></div><span className="completion-ring" style={{"--progress":`${prepPercent}%`} as React.CSSProperties}>{prepPercent}%</span></div><PreparationSharedPanel destination={activeTrip.destination} packingItems={packingItems} participants={expenseParticipants} accommodation={effectiveHotelDetails} showAccommodationCard={isSharedEditor} onPackingChange={updatePackingItem} onPackingAdd={addPackingItem} onPackingDelete={deletePackingItem} onCopy={(label,value)=>void copyText(label,value)}/></section>}
    ...
    {mapChoice&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>{if(!mapChoiceBusy)setMapChoice(null)}}><section className="upload-modal map-provider-modal" role="dialog" aria-modal="true" aria-label="選擇地圖" onMouseDown={event=>event.stopPropagation()}><div className="modal-handle"/><button className="modal-close" disabled={mapChoiceBusy} onClick={()=>setMapChoice(null)}><Icon name="close"/></button><span className="modal-icon map-provider"><Icon name={mapChoice.mode==="place"?"map":"route"} size={29}/></span><h2>選擇地圖</h2><p>{mapChoice.mode==="place"?"開啟行程位置":mapChoice.mode==="current"?`由你目前位置導航前往${mapChoice.destinationLabel}`:"顯示上一個行程前往目前行程的路線"}</p><div className="map-provider-actions"><button disabled={mapChoiceBusy} onClick={()=>void launchMap("google")}><strong>Google Maps</strong><small>全球地圖及交通路線</small></button><button disabled={mapChoiceBusy} onClick={()=>void launchMap("naver")}><strong>Naver Map</strong><small>{mapChoice.mode==="place"?"較適合韓國地址及地點":mapChoice.mode==="current"?`會先取得目前位置及${mapChoice.destinationLabel}定位`:"較適合韓國；會先定位兩個地點"}</small></button></div>{mapChoiceMessage&&<p className="map-provider-message" role="status">{mapChoiceMessage}</p>}</section></div>}

    {modal==="quickInfo"&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setModal(null)}><form className="upload-modal travel-quick-modal" onSubmit={saveTravelQuickInfo} onMouseDown={event=>event.stopPropagation()}><div className="modal-handle"/><button type="button" className="modal-close" onClick={()=>setModal(null)}><Icon name="close"/></button><span className="modal-icon quick-info"><Icon name="shield" size={29}/></span><h2>旅途中快速資料</h2><p>{activeTrip.name}・已存資料可離線查看，同行者亦會同步。</p>
      <section className="quick-info-section"><header><Icon name="hotel" size={18}/><strong>住宿</strong></header>{effectiveHotelDetails&&Object.values(effectiveHotelDetails).some(Boolean)?<div className="quick-info-rows"><div><span><small>酒店</small><strong>{effectiveHotelDetails.name||"未有酒店名稱"}</strong></span>{effectiveHotelDetails.name&&<button type="button" onClick={()=>void copyText("酒店名稱",effectiveHotelDetails.name)}>複製</button>}</div><div><span><small>地址</small><strong>{effectiveHotelDetails.address||"未有酒店地址"}</strong></span>{effectiveHotelDetails.address&&<button type="button" onClick={()=>void copyText("酒店地址",effectiveHotelDetails.address)}>複製</button>}</div><div><span><small>電話</small><strong>{effectiveHotelDetails.phone||"未有酒店電話"}</strong></span>{effectiveHotelDetails.phone&&<a href={`tel:${effectiveHotelDetails.phone.replace(/[^+\d]/g,"")}`}>致電</a>}</div></div>:<p className="quick-info-empty">請先在「準備」上傳或填寫酒店資料。</p>}</section>
      <section className="quick-info-section"><header><Icon name="plane" size={18}/><strong>航班與交通</strong></header>{quickFlightItems.length?<div className="quick-flight-list">{quickFlightItems.map(item=><article key={item.id}><time>{dateLabel(item.date)} {item.time||"待定"}</time><strong>{item.title}</strong><small>{item.note||item.location||""}</small></article>)}</div>:<p className="quick-info-empty">上傳機票並確認辨認資料後，航班時間會自動顯示。</p>}</section>
      {isKoreaDestination(activeTrip.destination)&&<section className="quick-info-section emergency"><header><Icon name="shield" size={18}/><strong>韓國緊急協助</strong></header><div className="emergency-call-grid"><a href="tel:112"><strong>112</strong><span>警察</span></a><a href="tel:119"><strong>119</strong><span>消防／救護</span></a><a href="tel:1330"><strong>1330</strong><span>旅遊熱線</span></a></div></section>}
      <section className="quick-info-section editable"><header><Icon name="receipt" size={18}/><strong>保險及緊急聯絡</strong></header><div className="quick-info-form"><label><span>保險公司</span><input name="insuranceProvider" defaultValue={activeTrip.quickInfo?.insuranceProvider??""} placeholder="例如：AIA"/></label><label><span>保單號碼</span><input name="policyNumber" defaultValue={activeTrip.quickInfo?.policyNumber??""} placeholder="旅行保險保單號碼"/></label><label><span>保險緊急電話</span><input name="insuranceHotline" type="tel" defaultValue={activeTrip.quickInfo?.insuranceHotline??""} placeholder="連國際區號"/></label><label><span>緊急聯絡人</span><input name="emergencyContactName" defaultValue={activeTrip.quickInfo?.emergencyContactName??""} placeholder="姓名"/></label><label><span>聯絡人電話</span><input name="emergencyContactPhone" type="tel" defaultValue={activeTrip.quickInfo?.emergencyContactPhone??""} placeholder="連國際區號"/></label></div>{(activeTrip.quickInfo?.insuranceHotline||activeTrip.quickInfo?.emergencyContactPhone)&&<div className="saved-call-actions">{activeTrip.quickInfo.insuranceHotline&&<a href={`tel:${activeTrip.quickInfo.insuranceHotline.replace(/[^+\d]/g,"")}`}>致電保險公司</a>}{activeTrip.quickInfo.emergencyContactPhone&&<a href={`tel:${activeTrip.quickInfo.emergencyContactPhone.replace(/[^+\d]/g,"")}`}>致電緊急聯絡人</a>}</div>}</section>
      <button className="confirm-button" type="submit">儲存快速資料</button></form></div>}
