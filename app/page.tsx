...
type MapChoice = {mode:"place";item:ItineraryItem}|{mode:"directions";from:ItineraryItem;to:ItineraryItem}|{mode:"hotel";to:ItineraryItem};
...
export default function Home(){
  ...
  const openHotelDirections=()=>{
    if(!hotelRouteOrigin){toast("請先在酒店資料補上酒店名稱或地址");return}
    setMapChoiceMessage("");setMapChoice({mode:"hotel",to:hotelRouteOrigin});
  };
  const launchMap=async(provider:"google"|"naver")=>{
    ...
    setMapChoiceMessage(mapChoice.mode==="hotel"?"正在取得目前位置及定位酒店…":"正在定位上一站及目前行程…");
    try{
      ...
      const originName=mapChoice.mode==="hotel"?"目前位置":queryFor(mapChoice.from);
      const [origin,destination]=await Promise.all([
        mapChoice.mode==="hotel"?currentPosition():findPlaceCoordinates(originName,GOOGLE_MAPS_KEY),
