...
function hotelItineraryItems(documentId:string,fields:OcrFields):ItineraryItem[]{
  ...
  const items:Array<ItineraryItem|null>=[
    fields.checkInDate&&fields.checkInTime?{id:`${documentId}-hotel-check-in`,date:fields.checkInDate,time:fields.checkInTime,title:`入住 ${hotel}`,note,type:"住宿",color:"blue"}:null,
    fields.checkOutDate&&fields.checkOutTime?{id:`${documentId}-hotel-check-out`,date:fields.checkOutDate,time:fields.checkOutTime,title:`退房 ${hotel}`,note,type:"住宿",color:"blue"}:null,
  ];
  return items.filter((item):item is ItineraryItem=>item!==null);
}
...
export default function Home(){
  ...
  const confirmOcr=(event:FormEvent<HTMLFormElement>)=>{
    ...
    setSelectedDate(ocrDocument.kind==="ticket"?(ocrFields.departureDate||activeTrip.startDate):(ocrFields.checkInDate||activeTrip.startDate));setModal(null);setOcrDocument(null);toast(ocrDocument.kind==="ticket"?"去程及回程時間已加入每日行程":"酒店資料及入住／退房時間已加入旅程");
  };
