...
export default function Home(){
  ...
  const openEditExpense=(expense:Expense)=>{setEditingExpenseId(expense.id);setConversionMode("rate");setExpenseDraft({...expense,exchangeRate:String(expense.currency==="HKD"?1:Number(expense.exchangeRate||0)),hkdAmount:String(expenseHkdAmount(expense))});setPicked("");setPickedFile(null);setReceiptRawText("");setReceiptProgress(0);if(receiptPreviewUrl){URL.revokeObjectURL(receiptPreviewUrl);setReceiptPreviewUrl("")}setModal("expense")};
  ...
  const saveExpense=(event:FormEvent<HTMLFormElement>)=>{
    ...
    const expense:Expense={...expenseDraft,merchant:expenseDraft.merchant.trim(),amount,exchangeRate,hkdAmount,id:editingExpenseId??crypto.randomUUID(),createdAt:editingExpenseId?(expenses.find(item=>item.id===editingExpenseId)?.createdAt??new Date().toISOString()):new Date().toISOString()};
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {modal==="expense"&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setModal(null)}>
      <form className="upload-modal trip-form expense-form" onSubmit={saveExpense} onMouseDown={event=>event.stopPropagation()}>
        ...
        <p>可以手動填寫，亦可即時拍照或從相簿選擇收據；外幣可輸入匯率，亦可直接填大約港幣金額。</p>
        ...
        <div className="form-grid expense-fields">
          ...
          <label><span>備註</span><textarea rows={3} value={expenseDraft.note} onChange={event=>setExpenseField("note",event.target.value)} placeholder="例如：晚餐、訂座或退款資料"/></label>
        </div>
        {receiptRawText&&<details className="ocr-raw"><summary>查看收據原始辨認文字</summary><pre>{receiptRawText}</pre></details>}<button className="confirm-button" type="submit">{editingExpenseId?"儲存更改":"確認入帳"}</button>
      </form>
    </div>}
