...
type Expense = {id:string;date:string;merchant:string;category:string;amount:number;currency:string;exchangeRate?:number;hkdAmount?:number;paidBy:string;splitMode:"none"|"equal";splitWith:string[];note:string;createdAt:string};
...
export default function Home(){
  ...
  const pick=(type:ModalType)=>{
    ...
    if(type==="receipt"){
      ...
      setEditingExpenseId(null);setExpenseDraft({date:dates.includes(today)?today:activeTrip.startDate,merchant:"",category:"餐飲",amount:0,currency:"HKD",exchangeRate:1,hkdAmount:0,paidBy:payer,splitMode:"none",splitWith:[],note:""});setReceiptRawText("");setReceiptProgress(0);setModal("expense");
  ...
  const openEditExpense=(expense:Expense)=>{setEditingExpenseId(expense.id);setExpenseDraft({...expense,exchangeRate:expense.currency==="HKD"?1:Number(expense.exchangeRate||0),hkdAmount:expenseHkdAmount(expense)});setPicked("");setPickedFile(null);setReceiptRawText("");setReceiptProgress(0);if(receiptPreviewUrl){URL.revokeObjectURL(receiptPreviewUrl);setReceiptPreviewUrl("")}setModal("expense")};
  ...
  const recognizeReceipt=async()=>{
    ...
    try{
      ...
      setExpenseDraft(current=>{const currency=fields.currency||current.currency;return {...current,merchant:fields.merchant||current.merchant,date:fields.date||current.date,amount:Number(fields.amount)||current.amount,currency,exchangeRate:currency==="HKD"?1:currency===current.currency?current.exchangeRate:0,hkdAmount:0}});
  ...
  const saveExpense=(event:FormEvent<HTMLFormElement>)=>{
    ...
    const exchangeRate=expenseDraft.currency==="HKD"?1:Number(expenseDraft.exchangeRate);
    if(!expenseDraft.merchant.trim()){toast("請填寫商戶或項目名稱");return}
    if(!Number.isFinite(amount)||amount<=0){toast("請填寫正確金額");return}
    if(expenseDraft.currency!=="HKD"&&(!Number.isFinite(exchangeRate)||exchangeRate<=0)){toast(`請填寫當時匯率：1 ${expenseDraft.currency} 等於幾多港幣`);return}
    if(expenseDraft.splitMode==="equal"&&expenseDraft.splitWith.length<2){toast("平均分帳最少要選擇兩位旅客");return}
    const expense:Expense={...expenseDraft,merchant:expenseDraft.merchant.trim(),amount,exchangeRate,hkdAmount:amount*exchangeRate,id:editingExpenseId??crypto.randomUUID(),createdAt:editingExpenseId?(expenses.find(item=>item.id===editingExpenseId)?.createdAt??new Date().toISOString()):new Date().toISOString()};
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {modal==="expense"&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setModal(null)}><form className="upload-modal trip-form expense-form" onSubmit={saveExpense} onMouseDown={event=>event.stopPropagation()}><div className="modal-handle"/><button type="button" className="modal-close" onClick={()=>setModal(null)}><Icon name="close"/></button><span className="modal-icon receipt"><Icon name={editingExpenseId?"edit":"receipt"} size={29}/></span><h2>{editingExpenseId?"編輯帳目":"新增帳目"}</h2><p>可以手動填寫，亦可即時拍照或從相簿選擇收據；外幣會按你輸入的當時匯率換算成港幣。</p>{!editingExpenseId&&<><input ref={receiptCameraRef} type="file" accept="image/*" capture="environment" hidden onChange={event=>chooseReceiptFile(event.target.files?.[0]??null)}/><input ref={receiptLibraryRef} type="file" accept="image/*" hidden onChange={event=>chooseReceiptFile(event.target.files?.[0]??null)}/>{receiptPreviewUrl?<button type="button" className="receipt-picker selected" onClick={()=>receiptLibraryRef.current?.click()}><img src={receiptPreviewUrl} alt="待辨認收據"/><span className="replace-receipt">更換相片</span></button>:<div className="receipt-source-actions"><button type="button" onClick={()=>receiptCameraRef.current?.click()}><Icon name="camera"/><strong>即時拍照</strong><small>開啟相機拍攝收據</small></button><button type="button" onClick={()=>receiptLibraryRef.current?.click()}><Icon name="upload"/><strong>從相簿選擇</strong><small>揀選已有收據相片</small></button></div>}{pickedFile&&<button type="button" className="receipt-recognize" disabled={receiptBusy} onClick={()=>void recognizeReceipt()}><Icon name="sparkle" size={16}/>{receiptBusy?`正在辨認 ${receiptProgress}%`:"辨認商戶、日期及總額"}</button>}</>}<div className="form-grid expense-fields"><label><span>商戶／項目 *</span><input value={expenseDraft.merchant} onChange={event=>setExpenseField("merchant",event.target.value)} placeholder="例如：韓國烤肉晚餐" required/></label><label><span>日期 *</span><input type="date" min={activeTrip.startDate} max={activeTrip.endDate} value={expenseDraft.date} onChange={event=>setExpenseField("date",event.target.value)} required/></label><label><span>類別</span><select value={expenseDraft.category} onChange={event=>setExpenseField("category",event.target.value)}><option>餐飲</option><option>交通</option><option>住宿</option><option>購物</option><option>景點</option><option>其他</option></select></label><label><span>貨幣</span><select value={expenseDraft.currency} onChange={event=>{const currency=event.target.value;setExpenseDraft(current=>({...current,currency,exchangeRate:currency==="HKD"?1:currency===current.currency?current.exchangeRate:0,hkdAmount:0}))}}><option>HKD</option><option>JPY</option><option>KRW</option><option>TWD</option><option>USD</option><option>EUR</option><option>CNY</option></select></label><label><span>原幣金額 *</span><input type="number" min="0.01" step="0.01" inputMode="decimal" value={expenseDraft.amount||""} onChange={event=>setExpenseField("amount",Number(event.target.value))} required/></label>{expenseDraft.currency!=="HKD"&&<label className="exchange-rate-field"><span>當時匯率（1 {expenseDraft.currency} = 幾多 HKD）*</span><input type="number" min="0.000001" step="any" inputMode="decimal" value={expenseDraft.exchangeRate||""} onChange={event=>setExpenseField("exchangeRate",Number(event.target.value))} placeholder="例如：0.0053" required/><small>請用交易當時匯率；系統不會自行估算。</small></label>}<div className="exchange-preview"><span>換算後港幣</span><strong>HKD {(Number(expenseDraft.amount||0)*(expenseDraft.currency==="HKD"?1:Number(expenseDraft.exchangeRate||0))).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong></div><label><span>付款人</span><select value={expenseDraft.paidBy} onChange={event=>setExpenseField("paidBy",event.target.value)}>{expenseParticipants.map(person=><option key={person} value={person}>{person===user?.email?.toLowerCase()?"我":person}</option>)}</select></label><label><span>分帳方式</span><select value={expenseDraft.splitMode} onChange={event=>{const mode=event.target.value as Expense["splitMode"];setExpenseDraft(current=>({...current,splitMode:mode,splitWith:mode==="equal"?(current.splitWith.length?current.splitWith:expenseParticipants):[]}))}}><option value="none">不分帳</option><option value="equal">同行者平均分帳</option></select></label>{expenseDraft.splitMode==="equal"&&<fieldset className="split-people"><legend>參與分帳的人</legend>{expenseParticipants.map(person=><label key={person}><input type="checkbox" checked={expenseDraft.splitWith.includes(person)} onChange={()=>toggleSplitPerson(person)}/><span>{person===user?.email?.toLowerCase()?"我":person}</span></label>)}</fieldset>}<label><span>備註</span><textarea rows={3} value={expenseDraft.note} onChange={event=>setExpenseField("note",event.target.value)} placeholder="例如：晚餐、訂座或退款資料"/></label></div>{receiptRawText&&<details className="ocr-raw"><summary>查看收據原始辨認文字</summary><pre>{receiptRawText}</pre></details>}<button className="confirm-button" type="submit">{editingExpenseId?"儲存更改":"確認入帳"}</button></form></div>}
