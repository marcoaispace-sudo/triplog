...
type MapChoice = {mode:"place";item:ItineraryItem;destination?:string}|{mode:"directions";from:ItineraryItem;to:ItineraryItem}|{mode:"current";to:ItineraryItem;destinationLabel:"酒店"|"機場"};
...
function JourneyReview({trip,onBack,onOpenDocument,onOpenExpense,onOpenMap}:{trip:Trip;onBack:()=>void;onOpenDocument:(document:TripDocument)=>void;onOpenExpense:(expense:Expense)=>void;onOpenMap:(item:ItineraryItem)=>void}){
  ...
  return <section className="screen-content journey-review"><div className="review-topbar"><button onClick={onBack}><Icon name="arrow" size={17}/>返回</button><span>旅程回顧</span></div>
    ...
    <div className="review-days">{tripDates(trip).map((date,index)=>{const items=itinerary.filter(item=>item.date===date);return <article className="review-day" key={date}><header><span>第 {index+1} 日</span><div><strong>{dateLabel(date)}</strong><small>{weekdayLabel(date)}・{items.length} 項安排</small></div></header><Timeline items={items} onMap={onOpenMap}/></article>})}</div>
...
export default function Home(){
  ...
  const currentTrip=useMemo(()=>trips.find(t=>t.id===activeTripId&&t.state==="active"&&t.endDate>=today)??trips.filter(t=>t.state==="active"&&t.endDate>=today).sort((a,b)=>a.startDate.localeCompare(b.startDate))[0]??null,[trips,activeTripId,today]);
  const activeTrip=useMemo(()=>currentTrip??trips.find(t=>t.state==="active")??defaultTrip,[trips,currentTrip]);
  ...
  const openMap=(item:ItineraryItem)=>{setMapChoiceMessage("");setMapChoice({mode:"place",item,destination:activeTrip.destination})};
  const openReviewMap=(item:ItineraryItem)=>{setMapChoiceMessage("");setMapChoice({mode:"place",item,destination:reviewTrip.destination})};
  ...
  const launchMap=async(provider:"google"|"naver")=>{
    ...
    const mapDestination=mapChoice.mode==="place"?(mapChoice.destination||activeTrip.destination):activeTrip.destination;
    const queryFor=(item:ItineraryItem)=>(item.location||`${item.title} ${mapDestination}`).trim();
    if(provider==="google"){
  ...
  return <main className="app-shell"><div className="phone-app">
    ...
    {active==="旅程"&&<TripManager trips={trips} activeTripId={currentTrip?.id??""} today={today} cloudState={cloudState} isSignedIn={Boolean(user)} onSelect={selectTrip} onAdd={()=>setModal("trip")} onArchive={id=>updateTripState(id,"archived")} onDelete={id=>updateTripState(id,"deleted")} onRestore={id=>updateTripState(id,"active")} onCompanions={openCompanionManager} onReview={openJourneyReview}/>}
    {active==="回顧"&&<JourneyReview trip={reviewTrip} onBack={()=>setActive("旅程")} onOpenDocument={document=>void openDocument(document)} onOpenExpense={openViewExpense} onOpenMap={openReviewMap}/>}
