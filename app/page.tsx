...
type ModalType = "ticket"|"hotel"|"receipt"|"trip"|"itinerary"|null;
...
type TripDocument = {id:string;kind:"ticket"|"hotel";name:string;path:string;size:number;createdAt:string};
type Trip = {id:string;name:string;destination:string;startDate:string;endDate:string;origin:string;destinationAirport:string;companions:number;budget:number;state:TripState;createdAt:string;deletedAt?:string;itinerary?:ItineraryItem[];requirements?:Requirement[];documents?:TripDocument[]};
...
function tripMetadata(trip:Trip){return JSON.stringify({triplog:{origin:trip.origin,destinationAirport:trip.destinationAirport,companions:trip.companions,budget:trip.budget,itinerary:trip.itinerary??[],requirements:trip.requirements,documents:trip.documents??[]}})}
function tripFromRow(row:TripRow):Trip{
  let meta:{triplog?:Partial<Pick<Trip,"origin"|"destinationAirport"|"companions"|"budget"|"itinerary"|"requirements"|"documents">>}={};
