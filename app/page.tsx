...
import { searchHotels, searchNearbyPlaces, type HotelPlaceResult, type NearbyPlaceResult } from "./google-places";
...
type ModalType = "ticket"|"hotel"|"receipt"|"trip"|"itinerary"|"ocr"|"recommend"|null;
...
export default function Home(){
  ...
  const openNearbyRecommendations=async()=>{
    ...
    setNearbyBusy(true);
    try{const places=await searchNearbyPlaces(base,GOOGLE_MAPS_KEY);setNearbyPlaces(places);if(!places.length)setNearbyMessage("附近暫時找不到合適推介，請稍後再試。")}catch(error){setNearbyMessage(error instanceof Error?error.message:"附近搜尋失敗，請稍後再試")}
